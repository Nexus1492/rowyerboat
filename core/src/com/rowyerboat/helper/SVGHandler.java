...
public class SVGHandler {
	...
	private static HashMap<String, Array<Location>> SVGs;
	...
	public static void start(String fileName, int width, int height, float precision) throws Exception {
		if (SVGs == null)
			SVGs = new HashMap<String, Array<Location>>();
		else if (SVGs.get(fileName) != null) {
			islands = SVGs.get(fileName);
			Gdx.app.log("SVGReader " + fileName, "Pre-parsed found.");
			return;
		}
		...
		if (svgClean.exists()) {
			Gdx.app.log("SVGReader " + fileName, "Clean file found");
