...
public class SVGReader {
	...
	public static void start(String loc, int width, int height, float precision) throws Exception {
		...
		if (svgClean.exists()) {
			Gdx.app.log("SVGReader", "Reading from file");
	...
	public static void transformSVG(final String content, int width, int height, float precision) throws Exception {
		...
		while (pathMatcher.find()) {
			++islandCount;
			concatPaths += (pathMatcher.group(1)) + "\\|";
		}
		String[] paths = concatPaths.substring(0, concatPaths.length() - 2).split("\\|");
		...
		for (int i = 0; i < islandCount; ++i) {
			...
			idMatcher = idPattern.matcher(paths[i]);
			String name = "";
			if (idMatcher.find())
				name = idMatcher.group(1);
			...
			while (xyMatcher.find()) {
				...
				// Beziercurve is read (x1y1, x2y2, x3y3)
				if (index == 6) {
	...
	private static void readFromFile(FileHandle svgClean) throws Exception {
		...
		for (int i = 1; i < lines.length; ++i) {
			...
			// process array
			String[] array = fields[1].substring(1, fields[1].length() - 2).split(", ");
			float[] verts = new float[Integer.parseInt(fields[2])];
			for (int j = 0; j < verts.length; ++j)
				verts[j] = Float.valueOf(array[j]);
			// process pos
			String[] posString = fields[3].substring(1, fields[3].length() - 2).split(",");
