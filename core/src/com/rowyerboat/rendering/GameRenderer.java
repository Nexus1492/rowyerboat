...
/**
 * Renderer containing and managing all relevant task to render the game to the
 * screen. Contains a GameUI instance and updates/calls its rendering function.
 * 
 * @author Roman Lamsal
 *
 */
public class GameRenderer {
	...
	/**
	 * used in the {@link GameRendererOpenGL} version of the game; is now used
	 * to switch between using a FBO (shaderID = 0) or non FBO rendering of the
	 * game
	 */
	...
	public void render(float delta) {
		...
		if (cameraMode != CameraMode.ortho) {
			...
			staticShaper.rect(0, 0, viewport3D.getWorldWidth(), viewport3D.getWorldHeight(), Color.BLUE, Color.BLUE,
		...
		boatInstance.transform.setTranslation(boatMid.x, boatMid.y, 0f).scale(boatScale, boatScale, boatScale);

		islandInstances.get(0).transform.scale(1f, islandScale, 1f);
		islandScale = 1f;
		arrowInstance.transform
				.setToTranslation(targetPos.x, targetPos.y, 30f + 10f * (MathUtils.sin(time * 10f) * 0.5f + 0.5f))
				.rotate(1, 0, 0, -90f);
		...
		for (int i = 0; i < islandInstances.size; ++i)
			modelBatch.render(islandInstances.get(i), environment, islandShader);
		modelBatch.end();
	...
	private void createModels() {
		...
		arrowModel = modelBuilder.createCone(20f, 50f, 20f, 15, new Material(ColorAttribute.createDiffuse(Color.RED)),
				Usage.Position);
		...
		/*
		 * not yet implemented properly will fill the instances array with the
		 * proper modelinstances (based on order)
		 */
		/*
		 * Model islandModel = modelLoader.loadModel(Gdx.files.getFileHandle(
		 * "models/lesserAntillesDemo.g3db", Files.FileType.Internal)); for (int
		 * i = 0; i < islands.size; ++i) { ModelInstance islandInstance = new
		 * ModelInstance(islandModel, islands.get(i).name");
		 * islandInstance.transform.setToRotation(1, 0, 0, 90f).scale(1f, 0.5f,
		 * 1f); islandInstances.add(islandInstance); } );
		 */
		...
		waterShader.init();
		Texture tex1 = AssetLoader.noise0, tex2 = AssetLoader.noise1, tex3 = AssetLoader.noise2,
		...
		meshPBuilder.rect(new Vector3(0, 0, -0.1f), new Vector3(world.width, 0, -0.1f),
				new Vector3(world.width, world.height, -0.1f), new Vector3(0, world.height, -0.1f),
				new Vector3(0, 0, 1));
		...
		case tutorial0:
		case tutorial1:
			islandShader = new DefaultShader(islandInstances.get(0).getRenderable(new Renderable()));
			break;
		default:
			islandShader = new Shader() {
				private ShaderProgram program;
				private int u_projTrans;
				private int u_worldTrans;
				private int u_cameraDir;

				@Override
				public void init() {
					program = new ShaderProgram(Gdx.files.internal("Shaders/islandVertex.glsl"),
							Gdx.files.internal("Shaders/islandFragment.glsl"));
					if (!program.isCompiled())
						Gdx.app.log("IslandShader", program.getLog());
				...
				public void begin(Camera camera, RenderContext context) {
					program.begin();
					program.setUniformf(u_cameraDir, camera.direction.nor());
	...
	public static enum CameraMode {
		toTarget, toTargetNear, boatDir, boatDirNear, dynamic, dynamicNear, dynamicTarget, dynamicTargetNear, ortho;
	...
	protected void debugCurrentGrid() {
		...
		for (int i = 0; i < (currentGrid != null ? currentGrid.length : 0); ++i)
			for (int j = 0; j < currentGrid[0].length; ++j) {
				...
				shaper.line(i * 100f, j * 100f, i * 100f + currentGrid[i][j].x * 100f,
						j * 100f + currentGrid[i][j].y * 100f);
			}
		shaper.end();
	}
	...
	protected void debugHitboxes() {
		...
		for (int i = 0; i < boat.getHitbox().getVertices().length; i += 2)
			shaper.circle(boat.getHitbox().getTransformedVertices()[i],
					boat.getHitbox().getTransformedVertices()[i + 1], 1);
		shaper.circle(boat.bowPoint.x, boat.bowPoint.y, .1f);
			...
			for (Polygon poly : locations.get(i).getTriangles()) {
				...
				for (int j = 0; j < 4; ++j)
					if (Intersector.isPointInPolygon(poly.getTransformedVertices(), 0, poly.getVertices().length,
							...
							boat.getHitbox().getTransformedVertices()[j * 2 + 1])) {
						shaper.setColor(Color.GREEN);
						shaper.polygon(poly.getTransformedVertices());
						shaper.set(ShapeType.Filled);
						shaper.circle(boat.getHitbox().getTransformedVertices()[j * 2],
								boat.getHitbox().getTransformedVertices()[j * 2 + 1], 1);
						shaper.setColor(Color.FIREBRICK);
		...
		shaper.line(boat.getPos().x + displace.x, boat.getPos().y + displace.y, boat.getPos().x, boat.getPos().y,
